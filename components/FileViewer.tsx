...
function DiffView({ patch }: { patch: string }) {
  ...
  if (!hasChanges) {
    return (
      <div style={{ padding: "12px 16px", fontSize: "var(--font-sm)", color: "var(--text-dim)", fontFamily: "var(--font-mono)" }}>
        {t("i18n.noChanges")}
      </div>
    );
  }
    ...
    >
      {segments.map((seg, si) => {
        ...
        const lines = seg.lines.map((line, li) => {
          ...
          const prefixColor =
            line.type === "added" ? "var(--state-success)" : line.type === "removed" ? "var(--state-danger)" : "var(--text-dim)";
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        <span
          ...
          style={{ display: "flex", alignItems: "center", gap: 4, color: watching ? "var(--state-success)" : "var(--text-dim)" }}
        >
          <span
            style={{
              width: 7,
              height: 7,
              borderRadius: "50%",
              background: watching ? "var(--state-success)" : "var(--border)",
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        <div style={{ width: "min(680px, 100%)" }}>
          {error && (
            <div style={{ color: "var(--state-danger)", fontSize: "var(--font-ui)", marginBottom: 12, textAlign: "center" }}>
              {error}
            </div>
          )}
          <audio
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        <span
          title={watching ? t("i18n.liveSync") : t("i18n.notWatching")}
          style={{ display: "flex", alignItems: "center", gap: 4, color: watching ? "var(--state-success)" : "var(--text-dim)", flexShrink: 0 }}
        >
          <span
            style={{
              width: 7,
      ...
      <div style={{ flex: 1, minHeight: 0, background: "var(--bg-panel)" }}>
        {error ? (
          <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, color: "var(--state-danger)", fontSize: "var(--font-ui)", textAlign: "center" }}>
            {error}
          </div>
        ) : (
          <iframe
...
function TextFileViewer({
  ...
  if (error && !isDeletedDiff) {
    return (
      <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--state-danger)", fontSize: 13 }}>
        {error}
      </div>
    );
  }
